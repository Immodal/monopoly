class Board {
    ...
    constructor(x, y, size) {
        ...
        this.tiles = []
        const insert = function(arr, x, y, w, h, side) { 
            const tile = { x: x, y: y, w: w, h: h, side: side, tile: Monopoly.TILES[arr.length] }
            arr.push(tile) 
            // Rects in tiles that show its color group
            if (tile.tile instanceof PropertyTile && Board.GROUP_COLORS[`${tile.tile.group}`]) {
                if (side==BOTTOM) {
        ...
        // Top Tiles
        insert(this.tiles, this.x, this.y, this.tileHeight, this.tileHeight, TOP)
        for (let i=0; i<Board.N_MID_TILES; i++) {
            insert(this.tiles, this.x+this.tileHeight+i*this.tileWidth, this.y, this.tileWidth, this.tileHeight, TOP)
        }
        // Right Tiles
        insert(this.tiles, this.x+this.tileHeight+Board.N_MID_TILES*this.tileWidth, y, 
            this.tileHeight, this.tileHeight, RIGHT)
        for (let i=0; i<Board.N_MID_TILES; i++) {
            insert(this.tiles, this.x+this.tileHeight+Board.N_MID_TILES*this.tileWidth, this.y+this.tileHeight+i*this.tileWidth, 
                this.tileHeight, this.tileWidth, RIGHT)
        }
        // Bottom Tiles
        insert(this.tiles, this.x+this.tileHeight+Board.N_MID_TILES*this.tileWidth, this.y+this.tileHeight+Board.N_MID_TILES*this.tileWidth, 
            this.tileHeight, this.tileHeight, BOTTOM)
        for (let i=Board.N_MID_TILES; i>0; i--) {
            insert(this.tiles, this.x+this.tileHeight+(i-1)*this.tileWidth, this.y+this.tileHeight+Board.N_MID_TILES*this.tileWidth, 
                this.tileWidth, this.tileHeight, BOTTOM)
        }
        // Left Tiles
        insert(this.tiles, this.x, this.y+this.tileHeight+Board.N_MID_TILES*this.tileWidth, this.tileHeight, this.tileHeight, LEFT)
        for (let i=Board.N_MID_TILES; i>0; i--) {
            insert(this.tiles, this.x, this.y+this.tileHeight+(i-1)*this.tileWidth, this.tileHeight, this.tileWidth, LEFT)
        }
    }
